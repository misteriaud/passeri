...
impl ControlPacket {
    pub fn new(name: String) -> Self {
        ControlPacket {
            initiator_token: rand::random::<u32>(),
            ssrc: 0,
            name: Some(name),
        }
    }
...
impl Frame {
    ...
    pub fn parse(src: &Vec<u8>) -> Result<Frame, Error> {
        ...
        match src[2..4] {
            [0x49, 0x4E] => Ok(Frame::IN(ControlPacket::parse_with_name(src)?)),
            [0x4F, 0x4B] => Ok(Frame::OK(ControlPacket::parse_with_name(src)?)),
            [0x4E, 0x4F] => Ok(Frame::NO(ControlPacket::parse(src)?)),
            [0x42, 0x59] => Ok(Frame::BY(ControlPacket::parse(src)?)),
            _ => Err(Error::Invalid),
        }
    }
}
...
fn get_u32(src: &[u8]) -> u32 {
    src.iter().fold(0, |acc, x| (acc << 8) + *x as u32)
}
